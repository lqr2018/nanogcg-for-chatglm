...
class Model:
    ...
    def __call__(self,
        ...
        参数:
            input_ids (torch.Tensor, optional): 输入的 token ID 序列。
            inputs_embeds (torch.Tensor, optional): 输入的嵌入向量。
            past_key_values (tuple, optional): 缓存的注意力键值对。
            use_cache (bool): 是否使用缓存。
            max_new_tokens (int): 生成的新 token 数量。

        返回:
            output_text (str): 生成的文本。
        """
        # 确保至少有一个输入（input_ids 或 inputs_embeds）
        if input_ids is None and inputs_embeds is None:
            raise ValueError("必须提供 input_ids 或 inputs_embeds 中的一个。")
        ...
        '''
        input_ids = self._get_ids(inputs_embeds)
        output = self.model(
            input_ids=input_ids,
            inputs_embeds=inputs_embeds
        )

        generated_logits = output.logits

        past_key_values = output.past_key_values

        # next_token = generated_logits.argmax(dim=-1)[:, -1].unsqueeze(-1)

        # generated_tokens = torch.tensor(generated_tokens, device=self.device)
        # generated_tokens = generated_tokens.transpose(0, 1)
    ...
    def _get_ids(self, inputs_embeds):
        sequence_length, batch_size, _ = inputs_embeds.shape
        pad_token_ids = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
...
class GCG:
    def __init__(
        ...
        self.model = Model(model, tokenizer)
        self.tokenizer = tokenizer
        self.config = config
        ...
        if self.config.probe_sampling_config:
            self.draft_model = Model(self.config.probe_sampling_config.draft_model, self.config.probe_sampling_config.draft_tokenizer)
    ...
    def run(
        ...
        for _ in tqdm(range(config.num_steps)):
            ...
            with torch.no_grad():
                ...
                print("new_search_width:", new_search_width)
            ...
            with torch.no_grad():
                ...
                if self.prefix_cache:
                    if not prefix_cache_batch or current_batch_size != search_batch_size:
                        ...
                        prefix_cache_batch = [[x.expand(-1, current_batch_size, -1, -1) for x in self.prefix_cache[i]]
                ...
                with torch.no_grad():
                    ...
                    if self.draft_prefix_cache:
                        if not draft_prefix_cache_batch or batch_size != search_batch_size:
                            ...
                            '''
                            draft_prefix_cache_batch = [
                                [x.expand(-1, batch_size, -1, -1) for x in self.draft_prefix_cache[i]]
                                for i in range(len(self.draft_prefix_cache))
                            ]
