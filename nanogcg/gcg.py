"""
This code is modified from https://github.com/GraySwanAI/nanoGCG
"""
...
class Model:
    ...
    def __call__(self,
        ...
        返回:
            output_text (tensor?): 模型的输出。
        """

        if self.model_type == "chatglm":
            return self._chatglm_generate(inputs_embeds, past_key_values, use_cache)
        else:
            return self._gpt_generate(inputs_embeds, past_key_values, use_cache)

    def _chatglm_generate(self,
                         inputs_embeds=None,
                         past_key_values=None,
                         use_cache=False):
                ...
                if self.prefix_cache:
                    if not prefix_cache_batch or current_batch_size != search_batch_size:
                        if self.model.model_type == "chatglm":
                            prefix_cache_batch = [
                                [x.expand(-1, current_batch_size, -1, -1) for x in self.prefix_cache[i]] for i in
                                range(len(self.prefix_cache))]
                        else:
                ...
                with torch.no_grad():
                    ...
                    if self.draft_prefix_cache:
                        if not draft_prefix_cache_batch or batch_size != search_batch_size:
                            if self.draft_model.model_type == "chatglm":
